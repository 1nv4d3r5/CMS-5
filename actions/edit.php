	...
	else if($type == 'events')
	{
		...
		else
		{
			...
			if (!$front_title || !$inner_title || !$front_content || !$inner_content) {
				...
				$result = 0;
				$sql = 'UPDATE events SET front_title = :front_title, inner_title = :inner_title, front_content = :front_content, inner_content = :inner_content, event_date = :event_date,	image_caption = :image_caption, modified_on = NOW(), modified_by = :modified_by, link = :link WHERE id = :id';
				$params = array(':front_title' => $front_title, ':inner_title' => $inner_title, ':front_content' => $front_content, 'inner_content' => $inner_content, ':event_date' => $event_date, ':image_caption' => $image_caption, ':id' => $value, ':modified_by' => $modified_by, ':link' => $link);
				$result = DatabaseHandler::Execute($sql, $params);
